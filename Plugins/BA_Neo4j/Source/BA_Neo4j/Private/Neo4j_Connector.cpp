...
UNeo4j_Connector* UNeo4j_Connector::CreateREST_API_ConnectionObject_NoRequest(const FString& UserName,
	const FString& Password, const FString& Url, const FString& Verb, TMap<FString, FString> &Headers,
	const FString& JsonBody)
{
	UNeo4j_Connector* const Proxy = NewObject<UNeo4j_Connector>();

	Proxy->Request.UserName = UserName;
	Proxy->Request.Password = Password;
	Proxy->Request.Url = Url;
	Proxy->Request.Verb = Verb;
	Proxy->Request.Headers = Headers;
	Proxy->Request.JsonBody = JsonBody;
...
void UNeo4j_Connector::StartRequest(const FString& UserName, const FString& Password, const FString& Url,
                                    const FString& Verb, TMap<FString, FString>& Headers, const FString& JsonBody)
{
		...
		mutable
			{
				...
				case EHttpRequestStatus::Succeeded:
					UE_LOG(LogTemp, Log, TEXT("Successful Neo4j API response: %s"), *(pResponse->GetContentAsString()));
...
void UNeo4j_Connector::StartRequest()
{
	this->StartRequest(
		Request.UserName,
		Request.Password,
